...
**Canonical upstream:** **[arunnadarasa/clinicalarc](https://github.com/arunnadarasa/clinicalarc)**. **This hackathon checkout / demo remote:** **[arunnadarasa/healtharcdemo](https://github.com/arunnadarasa/healtharcdemo)**. **Live demo app (Lovable):** **[https://clinipayarcdemo.lovable.app](https://clinipayarcdemo.lovable.app)** — HealthTech intelligence lanes, wallet, x402-paid APIs. **Marketing landing page:** **[clinipayarc.lovable.app](https://clinipayarc.lovable.app)**. **Live slide deck:** **[clinipayarcslides.lovable.app](https://clinipayarcslides.lovable.app)**. **Notes:** [`OPENCLAW_CLINICAL_HACKATHON_LEARNINGS.md`](./OPENCLAW_CLINICAL_HACKATHON_LEARNINGS.md). **Data safety:** use **dummy or synthetic** patient data in demos only — never real patient-identifiable data.
...
|-----------|------------------------------|
| **≤ $0.01 per paid API action** | x402 gates on neighbourhood + OpenEHR BFF + SNOMED RF2 paid POSTs use **$0.01** (see `server/neighbourhood/router.js`, `server/openehr/bffRouter.js`, `server/snomed/router.js`). The in-app transaction log shows **Cost (list)** **`$0.01`** for covered endpoints (`NEIGHBOURHOOD_X402_PRICE_DISPLAY` in `src/nhsTxHistory.ts`). |
...
| **Neighbourhood health plan (`/nhs/neighbourhood-insights`)** | **Hackathon app:** **openEHR** (EHRbase AQL via BFF), synthetic **artificial HES** LSOA aggregates, **SNOMED CT** ([IHTSDO](https://github.com/IHTSDO)), **Arc** + **USDC** (**x402**), optional **Featherless** narrative, **Circle Modular**, **Thirdweb** facilitator option where configured. The LSOA field **defaults to `E01022770`** so paid aggregates hit the indexed fast path (empty LSOA = full-table rollup — slow at scale; see [`docs/CLINICALARC_X402_LEARNINGS_AND_BEST_PRACTICES.md`](./docs/CLINICALARC_X402_LEARNINGS_AND_BEST_PRACTICES.md) pitfall **#17**). |
...
| **SNOMED intelligence (`/nhs/snomed-intelligence`)** | Local RF2 browser (`/api/snomed/rf2/*`): free GET search + concept detail; optional **paid POST** RF2 search and **paid POST** concept load (USDC x402 demo, same payloads as GET). |
...
**Lovable / portable sample (AE + OP + APC):** Handover trio **`docs/LOVABLE_ARTIFICIAL_HES_SAMPLE.{md,json,csv}`** + bulk CSVs under **`examples/lovable-artificial-hes/csv/`**. Regenerate CSVs with **`npm run hes:generate-lovable-sample`**.
...
- **Lovable static demo seeds (MD + JSON + CSV each):** [`docs/LOVABLE_SNOMED_DEMO_CONCEPTS.md`](./docs/LOVABLE_SNOMED_DEMO_CONCEPTS.md), [`docs/LOVABLE_DMD_DEMO_ITEMS.md`](./docs/LOVABLE_DMD_DEMO_ITEMS.md), [`docs/LOVABLE_NHS_UK_DATASET_LANE.md`](./docs/LOVABLE_NHS_UK_DATASET_LANE.md) (plus mini fixtures [`docs/lovable-nhs-uk-fixtures/`](./docs/lovable-nhs-uk-fixtures/)), [`docs/LOVABLE_ARTIFICIAL_HES_SAMPLE.md`](./docs/LOVABLE_ARTIFICIAL_HES_SAMPLE.md) (plus bulk ingest CSVs [`examples/lovable-artificial-hes/csv/`](./examples/lovable-artificial-hes/README.md)). Full app mega-handover: [`docs/LOVABLE_MEGA_HANDOVER_FULL_APP.md`](./docs/LOVABLE_MEGA_HANDOVER_FULL_APP.md).  
- **Landing / website handoff:** [`LOVABLE_LANDING_PAGE_CONTENT.md`](./LOVABLE_LANDING_PAGE_CONTENT.md), [`HEALTH_TECH_PROTOCOL_AZ.md`](./HEALTH_TECH_PROTOCOL_AZ.md), **demo:** [clinipayarcdemo.lovable.app](https://clinipayarcdemo.lovable.app), **marketing site:** [clinipayarc.lovable.app](https://clinipayarc.lovable.app)  
...
| `npm run hes:generate-lovable-sample` | Write **AE / OP / APC** sample CSVs under **`examples/lovable-artificial-hes/csv/`** (for Lovable / forks; tune counts via env) |
...
Then use `/api/snomed/rf2/health`, free GET `/api/snomed/rf2/search`, optional paid POST `/api/snomed/rf2/search` (same query body as GET query params, x402 demo parity), free GET `/api/snomed/rf2/concept/:sctid`, and optional paid POST `/api/snomed/rf2/concept` with JSON `{ "conceptId": "<sctid>" }` from the SNOMED page. Public **SNOMED International Browser** still works without Snowstorm.
...
- **Local RF2 index path:** If you need deterministic local browse/search without Docker/Elasticsearch, use the RF2 endpoints (`/api/snomed/rf2/*`). The first full-package ingest runs **in the background**; until the index is ready, **`/rf2/search`** and **`/rf2/concept`** return **503** with `buildStatus` (poll **`/rf2/health`** for `indexReady` / `indexBuildInFlight`). Ingest duration is file-size dependent (can be long on full UK+INT); after `indexReady`, searches are local SQLite + FTS.
...
| `/nhs/snomed-intelligence` | **SNOMED intelligence** — local RF2 browser (word + SCTID search, concept detail tabs) + free GET and optional paid POST RF2 **search** and **concept** with x402. |
...
- SNOMED page includes a **local RF2 browser pane** with search-by-term, direct SCTID lookup, tabbed concept details (summary, descriptions, hierarchy), **free GET** actions, optional **paid POST** x402 for the same search/concept data, and **loading/disabled** controls while RF2 API calls run.
- Backend local RF2 endpoints:
  - `GET /api/snomed/rf2/health` — includes `indexReady`, `indexBuildInFlight`, `buildStatus`, and stats when indexed.
  - `GET /api/snomed/rf2/search?q=...&limit=...` — **503** while index is building; otherwise FTS `MATCH` + grouped concept hits with bulk term resolution (not per-row N+1).
  - `POST /api/snomed/rf2/search` — **$0.01** x402 when `NHS_ENABLE_PAYMENT_GATE` is on; same semantics as GET; response adds `ok` + `receiptRef` around the search payload.
  - `GET /api/snomed/rf2/concept/:sctid` — **503** while not ready; otherwise descriptions + IS-A parents/children with **bulk** FSN/preferred-term resolution for related concepts.
  - `POST /api/snomed/rf2/concept` — **$0.01** x402 when gate on; JSON body `{ "conceptId": "<sctid>" }`; same concept shape as GET plus `ok` + `receiptRef`.
- Data source: extracted RF2 package root at `SNOMED_RF2_BASE_DIR` (override in `.env`; default may point at a local Downloads path — set explicitly in CI or other machines).
- Search relevance for demos: results ordered by **match count** per concept (not BM25), which is predictable and fast on large UK corpora.
